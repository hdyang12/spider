...
import com.yh.zhihu.model.ZhiHuBean;
import com.yh.zhihu.model.ZhiHuPicBean;
...
public class Spider {
	...
	public static void getTopUserInfo(String url) throws Exception {
		...
		for (page = 1;page<=50;page++){
			...
			List<String> userUrls = ZhiHuUtils.getUserUrl(nowUrl);
			// 循环访问链接并获取用户信息
	...
	public static void downloadCommend(String url){
		 // 访问链接并获取页面内容
		 String content = ZhiHuUtils.SendGet(url);
		 // 获取该页面的所有的知乎对象
		 ArrayList<ZhiHuBean> myZhihu = ZhiHuUtils.GetRecommendations(content);
